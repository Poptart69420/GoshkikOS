...
typedef struct vfs_node_struct
{
  ...
  uint64_t flags;
  uint64_t ref_count;
  ssize_t (*read)(struct vfs_node_struct *, void *buf, uint64_t off,
                  size_t count);
  ssize_t (*write)(struct vfs_node_struct *, void *buf, uint64_t off,
                   size_t count);
  ...
  int (*link)(struct vfs_node_struct *, const char *, struct vfs_node_struct *,
              const char *);
...
/// @brief mount a vfs_node to the specified path and create a directory fpr it
/// if needed
...
/// @param at
/// @param path the path (even if this absolute it will be interptreted as
/// relative)
/// @param flags open flags (VFS_READONLY,...)
...
ssize_t vfs_write(vfs_node *node, const void *buffer, uint64_t offset,
                  size_t count);
...
int vfs_auto_mount(const char *source, const char *target,
